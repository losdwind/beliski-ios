...
struct JournalItemView: View {
    ...
    var body: some View {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        if journal.content != nil {
                            TextContent(text: journal.content!)
                        }
                        
                        if journal.imageURLs != nil {
                            if journal.imageURLs!.count == 1 {
                                SingleImage(imageURL: journal.imageURLs![0])
                            } else {
                                ImageGrid(imageURLs: journal.imageURLs!)
                            }
                        }
                        
                        if journal.videoURLs != nil {
                            LazyVStack{
                                ForEach(journal.videoURLs!, id: \.self){
                                    videoURL in
                                    SingleVideo(videoURL: videoURL)
                                }
                            }
                        ...
                        HStack {
                            Time(time:journal.convertFIRTimestamptoString() ?? "Date N.A")
